...
class VolumeGenerator:
    ...
    def __init__(self, hypervolume, lattice, z_dim, invert, radius_mask, class_idx=0):
    ...
    def gen_volumes(self, outdir, z_values, suffix=None, write_zfile=True):
        """
        z_values: [nz, z_dim]
        """
        ...
        for i, z in enumerate(z_values):
            ...
            vol = models.eval_volume_method(self.hypervolume, self.lattice,
                                            ...
                                            radius=self.radius_mask, class_idx=self.class_idx)
...
class ModelAnalyzer:
    ...
    def __init__(self, config_vals: dict, train_config_vals: dict) -> None:
        ...
        hypervolume = nn.ModuleList([
            models.HyperVolume(**hypervolume_params)
            for _ in range(hypervolume_params['n_classes'])
            ])
        ...
        self.vg = VolumeGenerator(hypervolume, lattice, self.z_dim,
                                  True, radius_mask,
                                  class_idx=self.configs.class_idx)

        # load the conformations
        if self.train_configs.z_dim > 0:
            self.z = utils.load_pkl(
                os.path.join(self.traindir, f"conf.{self.epoch}.pkl"))
            if self.train_configs.n_classes > 1:
                predicted_p_classes = utils.load_pkl(
                    os.path.join(self.traindir, f"p_classes.{self.epoch}.pkl")
                )  # n_imgs, n_classes
                predicted_idx_best_class = np.argmax(predicted_p_classes, axis=-1)
                self.z = self.z[predicted_idx_best_class == self.configs.class_idx]
        ...
        self.outdir = os.path.join(self.traindir, f"analysis.epoch_{self.epoch}.class_{self.configs.class_idx}")
    ...
    def analyze(self):
        ...
        if self.train_configs.quick_config['capture_setup'] == 'spa':
            ...
            filter_ntbook['cells'][3]['source'] = filter_ntbook['cells'][3][
                'source'].replace("EPOCH = None", f"EPOCH = {self.epoch}")
        ...
        if self.configs.sample_z_idx is not None:
            ...
            for z_idx in self.configs.sample_z_idx:
        ...
        if self.configs.trajectory_1d is not None:
            self.logger.info(
                ...
                f"{self.configs.trajectory_1d[0]} to {self.configs.trajectory_1d[1]} "
                f"({self.configs.trajectory_1d[2]} samples)"
                )

            z_0 = self.z[self.configs.trajectory_1d[0]]
            z_1 = self.z[self.configs.trajectory_1d[1]]
            n_zs = self.configs.trajectory_1d[2]
            z_list = self.linear_interpolation(z_0, z_1, n_zs)

            trajdir = os.path.join(
                self.outdir, "trajectories",
                f"1d_{self.configs.trajectory_1d[0]}" + \
                f"_{self.configs.trajectory_1d[1]}_{self.configs.trajectory_1d[2]}"
                )
    ...
    def analyze_z0(self) -> None:
        assert self.z_dim == 0
        self.vg.gen_volumes(self.outdir, [None], write_zfile=False)

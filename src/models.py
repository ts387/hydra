...
class CryoDRGN3(nn.Module):
    def __init__(self, lattice, output_mask, n_particles_dataset, n_tilts_dataset, cnn_params, conf_regressor_params,
                 ...
                 ps_params=None, verbose_time=False, pretrain_with_gt_poses=False,
                 n_tilts_pose_search=1, n_classes=1):
        """
        lattice: Lattice
        output_mask: Mask
        ...
        self.coords = nn.Parameter(self.lattice.coords, requires_grad=False)  # [D * D, 3]
        ...
        if not use_gt_poses and will_use_point_estimates:
            self.pose_table = PoseTable(n_tilts_dataset, self.no_trans, self.D, use_gt_trans, n_classes=n_classes)
        ...
        if self.z_dim > 0:
            if cnn_params['conf']:
                ...
                self.conf_regressor = ConfRegressor(final_channels, final_size,
                                                    conf_regressor_params['z_dim'], conf_regressor_params['std_z_init'],
                                                    conf_regressor_params['variational'], n_classes=n_classes)

            else:
                self.conf_table = ConfTable(
                    n_particles_dataset, self.z_dim, conf_regressor_params['variational'],
                    conf_regressor_params['std_z_init'], n_classes=n_classes
                )

        # score
        if n_classes > 1:
            self.score_table = ScoreTable(n_particles_dataset, n_classes)
        self.n_classes = n_classes
        ...
        if not hyper_volume_params['explicit_volume']:
            self.hypervolume = nn.ModuleList([
                HyperVolume(self.D, self.z_dim, hyper_volume_params['n_layers'],
                            ...
                            pe_type_conf=hyper_volume_params['pe_type_conf'], n_classes=n_classes)
                for _ in range(n_classes)
            ])
        else:
            self.hypervolume = nn.ModuleList([
                VolumeExplicit(self.D, hyper_volume_params['domain'], hyper_volume_params['extent'],
                               n_classes=n_classes)
                for _ in range(n_classes)
            ])
    ...
    def forward(self, in_dict):
        ...
        out_dict:
            ...
            y_gt_processed: [batch_size, (n_tilts,) (n_classes,) n_pts]
        ...
        if self.verbose_time:
            out_dict['time_encoder'] = torch.tensor([start_time_decoder - start_time_encoder]).float().to(device)
            out_dict['time_decoder'] = torch.tensor([end_time - start_time_decoder]).float().to(device)
            out_dict['time_decoder_coords'] = torch.tensor([times['coords']]).float().to(device)
            out_dict['time_decoder_query'] = torch.tensor([times['query']]).float().to(device)
        for key in latent_variables_dict.keys():
            out_dict[key] = latent_variables_dict[key]
        if self.n_classes > 1:
            scores_dict = self.score_table(in_dict)
            for key in scores_dict.keys():
                out_dict[key] = scores_dict[key]
        return out_dict
    ...
    def encode(self, in_dict, ctf=None):
        ...
        if self.z_dim > 0:
            ...
            if self.pose_only:
                size = (batch_size, self.z_dim) if self.n_classes == 1 else (batch_size, self.n_classes, self.z_dim)
                z = self.std_z_init * torch.randn(size, dtype=torch.float32, device=device)
                conf_dict = {'z': z}
                if self.variational_conf:
                    logvar = torch.ones(size, dtype=torch.float32, device=device)
        ...
        if self.use_gt_poses or (self.pretrain and self.pretrain_with_gt_poses):
            rots = in_dict['R']
            if self.n_classes > 1:
                rots = rots[:, None].repeat(1, self.n_classes, 1, 1)
            pose_dict = {'R': rots}
            if not self.no_trans:
                trans = in_dict['t']
                if self.n_classes > 1:
                    trans = trans[:, None].repeat(1, self.n_classes, 1)
                pose_dict['t'] = trans
        ...
        # random poses
            ...
            pose_dict = {'R': lie_tools.random_rotmat(np.prod(in_dim) * self.n_classes, device=device)}
            pose_dict['R'] = pose_dict['R'].reshape(*in_dim, self.n_classes, 3, 3)
            if self.n_classes == 1:
                pose_dict['R'] = pose_dict['R'][..., 0, :, :]
            if not self.no_trans:
                pose_dict['t'] = torch.zeros((*in_dim, self.n_classes, 2)).float().to(device)
                if self.n_classes == 1:
                    pose_dict['t'] = pose_dict['t'][..., 0, :]

        # use pose search
        elif self.is_in_pose_search_step:
            self.hypervolume.eval()
            rot, trans, _ = pose_search.opt_theta_trans_multi_class(
                ...
                trans_search_factor=self.trans_search_factor, n_classes=self.n_classes
            )
            pose_dict = {
                'R': rot,
                'index': in_dict['index']
    ...
    def decode(self, latent_variables_dict, ctf_local, y_gt):
        ...
        output:
        [batch_size, (n_tilts,) (n_classes,) n_pts]
        [batch_size, (n_tilts,) (n_classes,) n_pts]
        dict {'coords': float, 'query': float}
        """
        rots = latent_variables_dict['R']
        in_shape = rots.shape[:-2]
        z = None
        ...
        if self.n_classes == 1:
            ...
            for k in range(self.n_classes):
                y_pred = self.hypervolume[k](x[..., k, :, :], z[:, k] if self.z_dim > 0 else None)  # batch_size, (n_tilts,) n_pts
                y_pred = self.apply_ctf(y_pred, ctf_local)
        ...
        if self.n_classes > 1:
            if y_gt.dim() == 3:
                y_gt = y_gt[:, None, :, :].repeat(1, self.n_classes, 1, 1)
            elif y_gt.dim() == 4:
                y_gt = y_gt[:, :, None, :, :].repeat(1, 1, self.n_classes, 1, 1)
        if not self.no_trans:
            trans = latent_variables_dict['t'].reshape(-1, 1, 2)
        ...
        return y_pred, y_gt_processed, times

    def eval_on_slice(self, x, z=None, class_idx=0):
        """
        ...
        if x.dim() == 4:
            ...
            y_pred = self.hypervolume[class_idx](x, z)
    ...
    def eval_volume(self, norm, zvals=[None], kvals=[0]):
        """
        norm: (mean, std)
        zvals: [[z_dim], [z_dim], ...] (size <= n_classes)
        kvals: list(int) (size <= n_classes)
        """
        vols = []
        for k, z in zip(kvals, zvals):
            vol = -1 * eval_volume_method(self.hypervolume, self.lattice, self.z_dim, norm, zval=z,
                                  radius=self.output_mask.current_radius, class_idx=k)
            vols.append(vol)
        return vols
...
def sample_conf(z_mu, z_logvar):
    """
    z_mu: [batch_size, (n_classes,) z_dim]
    z_logvar: [batch_size, (n_classes,) z_dim]

    output: [batch_size, (n_classes,) z_dim]
    """
...
def eval_volume_method(hypervolume, lattice, z_dim, norm, zval=None, radius=None, class_idx=0):
    """
...
class SharedCNN(nn.Module):
    def __init__(self, resolution, depth, channels, kernel_size, in_channels, nl=nn.ReLU, coord_conv=False,
                 dropout=False, radial_average=False):
        """
        resolution: int
        depth: int
        channels: int
...
class ScoreTable(nn.Module):
    def __init__(self, n_imgs, n_classes):
        """
        n_imgs: int
        n_classes: int
        """
        super(ScoreTable, self).__init__()
        self.table_score = nn.Parameter(torch.zeros(n_imgs, n_classes).float(), requires_grad=True)
    ...
    def forward(self, in_dict):
        ...
        output: dict
            ...
            idx_best_class: [batch_size]
        """
        p = self.softmax(self.table_score[in_dict['index']])
        idx_best_class = torch.argmax(p, dim=-1)
        return {'p': p, 'idx_best_class': idx_best_class}

class ConfTable(nn.Module):
    def __init__(self, n_imgs, z_dim, variational, std_z_init, n_classes=1):
        """
        n_imgs: int
        z_dim: int
        variational: bool
        ...
        size = (n_imgs, z_dim) if n_classes == 1 else (n_imgs, n_classes, z_dim)
        self.conf_init = torch.tensor(
            std_z_init * np.random.randn(*size)
        ).float()
        self.table_conf = nn.Parameter(self.conf_init, requires_grad=True)
        if variational:
            logvar_init = torch.tensor(np.ones(size)).float()
            self.table_logvar = nn.Parameter(logvar_init, requires_grad=True)

    def initialize(self, conf):
        """
        conf: [n_imgs, (n_classes,) z_dim] (numpy)
        """
    ...
    def forward(self, in_dict):
        ...
        output: dict
            z: [batch_size, (n_classes,) z_dim]
            z_logvar: [batch_size, (n_classes,) z_dim] if variational and not pose_only
        """
...
class PoseTable(nn.Module):
    def __init__(self, n_imgs, no_trans, resolution, use_gt_trans, n_classes=1):
        """
        n_imgs: int
        no_trans: bool
        resolution: int
        ...
        s2s2_init = np.array([1., 0., 0., 0., 1., 0.]).reshape(1, 6).repeat(n_imgs, axis=0)
        if n_classes > 1:
            s2s2_init = s2s2_init[:, None].repeat(n_classes, axis=1)
        s2s2_init = torch.tensor(s2s2_init).float()
        ...
        if not self.no_trans and not self.use_gt_trans:
            trans_init = np.zeros((n_imgs, 2))
            if n_classes > 1:
                trans_init = trans_init[:, None].repeat(n_classes, axis=1)
            trans_init = torch.tensor(trans_init).float()
            self.table_trans = nn.Parameter(trans_init, requires_grad=True)

    def initialize(self, rots, trans):
        """
        rots: [n_imgs, (n_classes,) 3, 3] (numpy)
        trans: [n_imgs, (n_classes,) 2] (numpy)
        """
        state_dict = self.state_dict()
    ...
    def forward(self, in_dict):
        ...
        if in_dict['y'].ndim == 4:
            pose_dict['R'] = pose_dict['R'].reshape(*in_dict['y'].shape[:-2], -1, 3, 3)
            if self.n_classes == 1:
                pose_dict['R'] = pose_dict['R'][..., 0, :, :]
            if not self.no_trans:
                pose_dict['t'] = pose_dict['t'].reshape(*in_dict['y'].shape[:-2], -1, 2)
                if self.n_classes == 1:
                    pose_dict['t'] = pose_dict['t'][..., 0, :]
        return pose_dict
...
class ConfRegressor(nn.Module):
    def __init__(self, channels, kernel_size, z_dim, std_z_init, variational, n_classes=1):
        """
        channels: int
        kernel_size: int
        z_dim: int
        ...
        if variational:
            out_features = 2 * n_classes * z_dim
        else:
            out_features = n_classes * z_dim
    ...
    def forward(self, shared_features):
        ...
        output: dict
            z: [..., (n_classes,) z_dim]
            z_logvar: [..., (n_classes,) z_dim] if variational and not pose_only
        """
        ...
        if self.variational:
            z = z_full[:, :(self.n_classes * self.z_dim)]
            z_logvar = nn.Tanh()(z_full[:, (self.n_classes * self.z_dim):] / 10.) * 10.
            if self.n_classes > 1:
                z = z.reshape(-1, self.n_classes, self.z_dim)
                z_logvar = z_logvar.reshape(-1, self.n_classes, self.z_dim)
            conf_dict = {
                'z': z,
                'z_logvar': z_logvar
            }
            ...
            if self.n_classes > 1:
                z = z_full.reshape(-1, self.n_classes, self.z_dim)
            conf_dict = {
                'z': z
            }
        return conf_dict
...
class VolumeExplicit(nn.Module):
    def __init__(self, resolution, domain, extent, n_classes=1):
        """
        resolution: int
        domain: str
        extent: float
